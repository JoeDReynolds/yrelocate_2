...
def remove_population_collection():
    db_population = app.config["pymongo"].db['population']
...
def insert_population_data(recreate_db = False):
    ...
    try:
        ...
        for column in population_df:
            ...
            if (app.config['pymongo'].db['population']
                        .find({"place_id": location.raw["place_id"]})
                        .count() == 0):
                print(records)
                app.config['pymongo'].db['population'].insert(records)
            else:
                print("updating")
            app.config['pymongo'].db_['population'].update({"place_id": records["place_id"]},
                records, upsert=True)
